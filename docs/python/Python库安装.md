...
如果是第一次开发weso项目，可以直接利用`debug`模式自动创建`project.json`，即通过`-d`参数。
```bash
weso.exe -d
```
执行上面命令，默认在当前路径下创建`weso`项目，并且会回自动创建`project.json`.

如果希望在指定路径创建项目，则通过`-w`参数指定：
```bash
weso.exe -d -w C:\path\to\your\project
```
