...
1. 读取 `weso.json` 配置。
2. 若 `python/src/` 有 `.py`，编译为 `.pyc` 后随包下发（源码不进包）。
3. res 与 python 随包打入 exe（若存在 res/ 或 python/）。
4. `www/` 随包打入 exe。
...
2. 将 res 与 python 解压到用户选择的安装路径（并列落地）。
3. 把精简后的 exe 复制到安装路径。
4. 在桌面和开始菜单创建快捷方式，指向已安装的 exe。
5. 自动启动已安装副本，安装包进程退出。

已安装的 exe 再次双击直接进入应用模式（无需安装）。
无 res/python 的纯 www 应用打包后双击也直接运行。
