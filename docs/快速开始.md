# 快速开始
# 1.开发环境
前往[https://huachao1001.github.io/weso](https://huachao1001.github.io/weso/)下载最新的`weso`打包工具到本地，得到如下文件：
```
weso.exe
```
保存到本地，本文以`C:\weso.exe`为例。

# 2.配置文件
指定开发工作目录，以`D:\MyWork\FirstProj`为例，执行如下命令：
```
C:\weso.exe -d -w D:\MyWork\FirstProj
```
...
```json
{
	...
	"copyright":"Copyright(C) 2024-2025 weso",
...
```
`weso`提供了很多`Native`能力，并且还在持续扩充中。`weso`可以直接调用`dll`中的函数，为开发者提供了`js`调用`C/C++`能力，开发者可以将复杂耗时的功能用`C/C++`实现，`js+css`实现酷炫的界面。以调用`Windows`的系统提示窗口为例，修改`index.html`如下：
```html
<html>
...
执行如下命令将脚本打包成exe
```bash
C:\weso.exe -p -w D:\MyWork\FirstProj
```
