...
本仓库是 Weso 的**示例程序**——一个内置代码编辑器与输出日志的交互式 Playground。左侧菜单按模块组织 59 个可运行脚本，选中即可查看代码、点击运行并实时观察原生调用结果。
...
| 模块 | 示例数 | 覆盖能力 |
|------|--------|---------|
| ⚙️ 系统 | 8 | 原生消息框、执行 cmd 命令、环境变量读写、捕获 stdout/stderr、DevTools、退出应用、开机自启、运行时回调事件 |
| 📁 文件 | 17 | 文本/二进制/base64 读写、多行读取、目录创建与列举、文件选择器、路径操作、资源包读取、文件拖拽监听 |
| 🪟 窗口 | 16 | 显示/隐藏/最小化/最大化、创建与销毁子窗口、窗口句柄与屏幕几何、拖拽移动、关闭拦截、系统托盘、窗口间通信、键盘/鼠标 Hook、injectJS 注入在线网站、右下角桌面萌宠 |
| ➕ C++ | 14 | DLL 一次性调用、整数/浮点/字符串/混合类型传参、句柄加载与地址调用、`Dll` 封装类、DLL→JS 消息回推 |
...
```
weso-github/
├── www/                  # Web 资产（界面 + 示例脚本 + 文档）
│   ├── index.html        # 示例入口页面
│   ├── docs.html         # 使用文档入口页面
│   ├── index/            # 示例主界面逻辑、菜单配置、样式
│   ├── docs/             # 文档页逻辑与样式
│   ├── code/             # 59 个分类示例脚本
│   │   ├── system/       #   系统能力
│   │   ├── file/         #   文件 I/O
...
│   │   ├── cpp/          #   DLL 互操作
│   │   └── python/       #   Python 集成
│   ├── skills/           # Weso 开发参考文档（使用文档页渲染）
│   └── lib/              # CodeMirror 代码编辑器 + markdown/highlight 工具
...
// 系统与 OS
W.alert("原生消息框");
await W.system("echo hello");                 // 执行 cmd，异步返回 stdout+stderr
...
完整类型定义见 [`weso.d.ts`](weso.d.ts)，开发参考文档见 [`www/skills/`](www/skills/)。
