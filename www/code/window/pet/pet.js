...
console.log("  createWin({transparent:true})  创建即透明 (真透明背景的关键)");
console.log("  setTransparent(true, hwnd)     启用透明, 透明像素直显桌面");
console.log("  setAlwaysOnTop(true, hwnd)     始终置顶, 全屏窗口也盖不住");
console.log("  setShadow(false, hwnd)         显式关阴影 (覆盖透明窗口默认联动)");
...
// 1) 创建即透明的无边框子窗口
//    transparent:true 是真透明的关键: 窗口创建时即启用真透明背景,
//    透明像素直显桌面。运行时 setTransparent 无法把非透明创建的窗口补成真透明。
...
//    (透明窗口创建时已默认关阴影并启用透明, 这里幂等显式调用, 便于演示 API)
...
console.log("  setShadow(true, " + hwnd + ")          强制开阴影 (透明窗口加方形光晕)");
